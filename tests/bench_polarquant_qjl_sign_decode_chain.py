...
from turboquant.turboquant_qjl_sign_decode_chain_cuda import (
    turboquant_qjl_shift_decode_fused_logits_cuda,
    turboquant_qjl_xor_signflip_fused_logits_cuda,
    turboquant_qjl_shift_decode_xor_signflip_fused_logits_cuda,
)
...
def benchmark_one_length(
    *,
    ...
    def shift_decode() -> torch.Tensor:
        return turboquant_qjl_shift_decode_fused_logits_cuda(**common)

    def xor_signflip() -> torch.Tensor:
        return turboquant_qjl_xor_signflip_fused_logits_cuda(**common)

    def shift_decode_xor_signflip() -> torch.Tensor:
        return turboquant_qjl_shift_decode_xor_signflip_fused_logits_cuda(**common)
    ...
    shift_ms, shift_out = bench_cuda_ms(
        shift_decode,
        warmup=warmup,
        iters=iters,
    )
    xor_ms, xor_out = bench_cuda_ms(
        xor_signflip,
        warmup=warmup,
        iters=iters,
    )
    shift_xor_ms, shift_xor_out = bench_cuda_ms(
        shift_decode_xor_signflip,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "qjl_shift_decode_ms": float(shift_ms),
            "qjl_xor_signflip_ms": float(xor_ms),
            "qjl_shift_decode_xor_signflip_ms": float(shift_xor_ms),
        },
        "speedup_vs_baseline": {
            "qjl_shift_decode_over_baseline": float(baseline_ms / shift_ms),
            "qjl_xor_signflip_over_baseline": float(baseline_ms / xor_ms),
            "qjl_shift_decode_xor_signflip_over_baseline": float(
                baseline_ms / shift_xor_ms
            ),
        },
        "parity_vs_baseline": {
            "qjl_shift_decode": parity_metrics(shift_out, baseline_out),
            "qjl_xor_signflip": parity_metrics(xor_out, baseline_out),
            "qjl_shift_decode_xor_signflip": parity_metrics(
                shift_xor_out,
                baseline_out,
            ),
        },
        "ablation_notes": {
            "qjl_shift_decode": (
                "Same lane-nibble storage. Replaces low/high-nibble ternary "
                "with dynamic 0-or-4-bit shift."
            ),
            "qjl_xor_signflip": (
                "Same lane-nibble storage. Replaces ternary +/- sign "
                "application with FP32 sign-bit XOR."
            ),
            "qjl_shift_decode_xor_signflip": (
                "Combines dynamic-shift nibble extraction and FP32 sign-bit XOR."
            ),
        },
    }
    ...
    del baseline_out, shift_out, xor_out, shift_xor_out
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "QJL sign decode instruction-chain reduction benchmark for the current "
            "FP16 L2 LUT + lane-nibble QJL + early-radii + early-qjl-norm full fused path."
        )
    )
    p.add_argument(
        "--seq_lens",
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current QJL sign decode fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current QJL sign decode fast paths require "
            "D=128, M=128, num_levels=4."
        )

    print("========== QJL sign decode instruction-chain reduction benchmark ==========")
    ...
    print(
        ...
        "+ early-qjl-norm. Candidates keep the same storage and exact math, "
        "rewriting only the QJL sign-byte decode / sign-application chain."
    )
    ...
    payload = {
        "benchmark": "polarquant_qjl_sign_decode_instruction_chain_reduction",
        "method": (
            "current_best_vs_qjl_shift_decode_vs_qjl_xor_signflip_vs_"
            "qjl_shift_decode_xor_signflip"
        ),
        "device": str(device),
        "config": {
    ...
    print("[PASS] QJL sign decode instruction-chain reduction benchmark completed.")
