...
def benchmark_one_length(
    *,
    ...
    def polar_early() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_early_radii_polar_only_cuda(
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
            centroids_l3=codebooks.centroids[2],
    ...
    def polar_vector() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_vector_radii_polar_only_cuda(
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
            centroids_l3=codebooks.centroids[2],
    ...
    def full_early() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_early_radii_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    def full_vector() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_vector_radii_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    polar_base_ms, polar_base_out = bench_cuda_ms(polar_base, warmup=warmup, iters=iters)
    polar_early_ms, polar_early_out = bench_cuda_ms(polar_early, warmup=warmup, iters=iters)
    polar_vector_ms, polar_vector_out = bench_cuda_ms(polar_vector, warmup=warmup, iters=iters)

    full_base_ms, full_base_out = bench_cuda_ms(full_base, warmup=warmup, iters=iters)
    full_early_ms, full_early_out = bench_cuda_ms(full_early, warmup=warmup, iters=iters)
    full_vector_ms, full_vector_out = bench_cuda_ms(full_vector, warmup=warmup, iters=iters)

    result = {
        "seq_len": int(seq_len),
        "shape": {"B": B, "H": H, "Q": Q, "D": D, "M": M},
        "timing_ms": {
            "polar_fp16_l2_baseline_ms": float(polar_base_ms),
            "polar_fp16_l2_early_radii_ms": float(polar_early_ms),
            "polar_fp16_l2_vector_radii_ms": float(polar_vector_ms),
            "full_fp16_l2_lane_nibble_baseline_ms": float(full_base_ms),
            "full_fp16_l2_lane_nibble_early_radii_ms": float(full_early_ms),
            "full_fp16_l2_lane_nibble_vector_radii_ms": float(full_vector_ms),
        },
        "speedup_vs_baseline": {
            "polar_early_radii_over_baseline": float(polar_base_ms / polar_early_ms),
            "polar_vector_radii_over_baseline": float(polar_base_ms / polar_vector_ms),
            "full_early_radii_over_baseline": float(full_base_ms / full_early_ms),
            "full_vector_radii_over_baseline": float(full_base_ms / full_vector_ms),
        },
        "parity_vs_baseline": {
            "polar_early_radii": parity_metrics(polar_early_out, polar_base_out),
            "polar_vector_radii": parity_metrics(polar_vector_out, polar_base_out),
            "full_early_radii": parity_metrics(full_early_out, full_base_out),
            "full_vector_radii": parity_metrics(full_vector_out, full_base_out),
        },
    }
    ...
    del polar_base_out, polar_early_out, polar_vector_out
    del full_base_out, full_early_out, full_vector_out
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Radii access optimization benchmark for the current best "
            "L2-combined-FP16-LUT + lane-nibble-QJL score path."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current radii optimization fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current radii optimization fast paths require D=128, M=128, num_levels=4.")

    print("========== Radii access optimization benchmark ==========")
    ...
    print()
    print(
        "[Note] Compares the current FP16 L2 LUT baseline against two radii "
        "variants: early radii load and vectorized warp radii pair loads."
    )
    ...
    payload = {
        "benchmark": "polarquant_radii_access_optimization",
        "method": "early_radii_vs_vectorized_radii_on_fp16_l2_lut_lane_nibble",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] Radii access optimization benchmark completed.")
