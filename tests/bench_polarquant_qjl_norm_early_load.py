...
def benchmark_one_length(
    *,
    ...
    def early_qjl_norm() -> torch.Tensor:
    ...
    result = {
        ...
        "timing_ms": {
            "full_early_radii_baseline_ms": float(baseline_ms),
            "full_early_radii_early_qjl_norm_ms": float(candidate_ms),
        },
        "speedup_vs_early_radii_baseline": {
            "full_early_qjl_norm_over_baseline": float(baseline_ms / candidate_ms),
        },
        "parity_vs_early_radii_baseline": {
            "full_early_qjl_norm": parity_metrics(candidate_out, baseline_out),
        },
    }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Early qjl_norm load benchmark for the current "
            "L2-combined-FP16-LUT + lane-nibble-QJL + early-radii score path."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current early-qjl-norm fast path requires B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current early-qjl-norm fast path requires D=128, M=128, num_levels=4.")

    print("========== Early qjl_norm load benchmark ==========")
    ...
    print()
    print(
        "[Note] Baseline is the current FP16 L2 LUT + lane-nibble QJL + early-radii path. "
        "Candidate hoists qjl_norms[key] from the final output stage to the beginning "
        "of the valid-token body."
    )
    ...
    payload = {
        "benchmark": "polarquant_early_qjl_norm_load",
        "method": "early_radii_baseline_vs_early_radii_plus_early_qjl_norm",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] Early qjl_norm load benchmark completed.")
