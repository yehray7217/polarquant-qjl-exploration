...
from turboquant.turboquant_logits_tree_l1_lut_lane_nibble_cuda import (
    turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_cuda,
)
from turboquant.turboquant_factor_lut_shared_staging_cuda import (
    turboquant_polar_tree_l1_lut_shared_polar_only_cuda,
    turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_cuda,
)
...
def benchmark_one_length(
    *,
    ...
    def polar_global_lut() -> torch.Tensor:
    ...
    def polar_shared_lut() -> torch.Tensor:
        return turboquant_polar_tree_l1_lut_shared_polar_only_cuda(
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l1_factor_lut=l1_factor_lut,
            centroids_l2=codebooks.centroids[1],
    ...
    def full_global_lut() -> torch.Tensor:
        return turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_cuda(
    ...
    def full_shared_lut() -> torch.Tensor:
        return turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l1_factor_lut=l1_factor_lut,
    ...
    polar_global_ms, polar_global_scores = bench_cuda_ms(
        polar_global_lut,
        warmup=warmup,
        iters=iters,
    )
    polar_shared_ms, polar_shared_scores = bench_cuda_ms(
        polar_shared_lut,
        warmup=warmup,
        iters=iters,
    )
    full_global_ms, full_global_scores = bench_cuda_ms(
        full_global_lut,
        warmup=warmup,
        iters=iters,
    )
    full_shared_ms, full_shared_scores = bench_cuda_ms(
        full_shared_lut,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            "polar_only_global_factor_lut_ms": float(polar_global_ms),
            "polar_only_shared_factor_lut_ms": float(polar_shared_ms),
            "full_lane_nibble_global_factor_lut_ms": float(full_global_ms),
            "full_lane_nibble_shared_factor_lut_ms": float(full_shared_ms),
        },
        "speedup": {
            "polar_shared_over_polar_global": float(polar_global_ms / polar_shared_ms),
            "full_shared_over_full_global": float(full_global_ms / full_shared_ms),
        },
        "parity": {
            "polar_shared_vs_polar_global": parity_metrics(
                polar_shared_scores,
                polar_global_scores,
            ),
            "full_shared_vs_full_global": parity_metrics(
                full_shared_scores,
                full_global_scores,
            ),
        },
        "shared_factor_lut_bytes_per_cta": 4096,
    }
    ...
    del q_projected, l1_factor_lut, lane_nibble_qjl_signs
    del polar_global_scores, polar_shared_scores
    del full_global_scores, full_shared_scores
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Factor LUT shared-memory staging benchmark: "
            "Polar-only and current full lane-nibble fused paths."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current shared-factor-LUT fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current shared-factor-LUT fast paths require D=128, M=128, num_levels=4.")

    print("========== Factor LUT shared-memory staging benchmark ==========")
    ...
    print()
    print(
        "[Note] Each CTA stages the 4 KB head-local L1 factor LUT "
        "into shared memory before scoring its 8 tokens."
    )
    ...
    payload = {
        "benchmark": "polarquant_factor_lut_shared_staging",
        "method": "polar_l1_lut_shared_factor_lut_cta_staging",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] Factor LUT shared-memory staging benchmark completed.")
