...
from turboquant.turboquant_meta_segmented_decode_cuda import (
...
def benchmark_one_length(
    *,
    ...
    def trimmed_meta12() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    def segmented_meta_l1l2() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    trimmed_ms, trimmed_out = bench_cuda_ms(
        trimmed_meta12,
        warmup=warmup,
        iters=iters,
    )
    segmented_ms, segmented_out = bench_cuda_ms(
        segmented_meta_l1l2,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "full_trimmed_meta12_ms": float(trimmed_ms),
            "full_segmented_meta_l1l2_ms": float(segmented_ms),
        },
        "speedup_vs_early_qjl_norm_baseline": {
            "full_trimmed_meta12_over_baseline": float(
                baseline_ms / trimmed_ms
            ),
            "full_segmented_meta_l1l2_over_baseline": float(
                baseline_ms / segmented_ms
            ),
        },
        "parity_vs_early_qjl_norm_baseline": {
            "full_trimmed_meta12": parity_metrics(
                trimmed_out,
                baseline_out,
            ),
            "full_segmented_meta_l1l2": parity_metrics(
                segmented_out,
                baseline_out,
            ),
        },
    ...
    del baseline_out, trimmed_out, segmented_out
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Segmented packed-meta decode benchmark for the current "
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current segmented-meta fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current segmented-meta fast paths require "
            "D=128, M=128, num_levels=4."
        )

    print("========== Segmented packed-meta decode benchmark ==========")
    ...
    print(
        ...
        "+ early-qjl-norm. Candidate A trims packed-meta load from words "
        "0..15 to used words 0..11. Candidate B segments decode: early L1/L2 "
        "words 0..9, late L3/L4 words 10..11."
    )
    ...
    payload = {
        "benchmark": "polarquant_segmented_packed_meta_decode",
        "method": (
            "early_qjl_norm_baseline_vs_trimmed_meta12_vs_"
            "segmented_meta_l1l2_late_l3l4"
        ),
        "device": str(device),
        "config": {
            "B": B,
    ...
    print("[PASS] Segmented packed-meta decode benchmark completed.")
