...
def benchmark_one_length(
    *,
    ...
    def full_l1_lut_meta64_qjl() -> torch.Tensor:
    ...
    def full_l1_lut_lane_nibble_qjl() -> torch.Tensor:
    ...
    meta64_ms, meta64_scores = bench_cuda_ms(
        full_l1_lut_meta64_qjl,
        warmup=warmup,
        iters=iters,
    )
    lane_nibble_ms, lane_nibble_scores = bench_cuda_ms(
        full_l1_lut_lane_nibble_qjl,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            "full_l1_lut_meta64_qjl_ms": float(meta64_ms),
            "full_l1_lut_lane_nibble_qjl_ms": float(lane_nibble_ms),
        },
        "speedup": {
            "lane_nibble_full_over_meta64_full": float(meta64_ms / lane_nibble_ms),
        },
        "parity_lane_nibble_full_vs_meta64_full": parity_metrics(
            lane_nibble_scores,
            meta64_scores,
        ),
        "extra_lane_nibble_sign_storage_bytes": int(tensor_bytes(lane_nibble_qjl_signs)),
    }
    ...
    del l1_factor_lut, lane_nibble_qjl_signs
    del meta64_scores, lane_nibble_scores
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Full L1-LUT fused logits benchmark: "
            "original meta64 QJL signs vs lane-nibble QJL signs."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current lane-nibble fused fast path requires B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current lane-nibble fused fast path requires D=128, M=128, num_levels=4.")

    print("========== PolarQuant L1-LUT full fused lane-nibble QJL benchmark ==========")
    ...
    print()
    print(
        "[Note] Polar L1-LUT math is unchanged. "
        "Only the fused kernel's QJL residual sign lookup switches from "
        "meta64 embedded signs to lane-nibble signs."
    )
    ...
    payload = {
        "benchmark": "polarquant_tree_l1_lut_lane_nibble_full_fused",
        "method": "full_l1_lut_meta64_qjl_vs_lane_nibble_qjl",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] PolarQuant L1-LUT full fused lane-nibble QJL benchmark completed.")
