...
from turboquant.turboquant_tree_late_compact_predication_cuda import (
    turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_cuda,
    turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_cuda,
)
...
def benchmark_one_length(
    *,
    ...
    def polar_baseline() -> torch.Tensor:
    ...
    def polar_compact() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_cuda(
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
            centroids_l3=codebooks.centroids[2],
    ...
    def full_baseline() -> torch.Tensor:
    ...
    def full_compact() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    polar_base_ms, polar_base_out = bench_cuda_ms(
        polar_baseline, warmup=warmup, iters=iters
    )
    polar_compact_ms, polar_compact_out = bench_cuda_ms(
        polar_compact, warmup=warmup, iters=iters
    )
    full_base_ms, full_base_out = bench_cuda_ms(
        full_baseline, warmup=warmup, iters=iters
    )
    full_compact_ms, full_compact_out = bench_cuda_ms(
        full_compact, warmup=warmup, iters=iters
    )
    ...
    result = {
        ...
        "timing_ms": {
            "polar_early_radii_baseline_ms": float(polar_base_ms),
            "polar_compact_late_tree_ms": float(polar_compact_ms),
            "full_early_radii_baseline_ms": float(full_base_ms),
            "full_compact_late_tree_ms": float(full_compact_ms),
        },
        "speedup_vs_early_radii_baseline": {
            "polar_compact_late_tree_over_baseline": float(polar_base_ms / polar_compact_ms),
            "full_compact_late_tree_over_baseline": float(full_base_ms / full_compact_ms),
        },
        "parity_vs_early_radii_baseline": {
            "polar_compact_late_tree": parity_metrics(
                polar_compact_out, polar_base_out
            ),
            "full_compact_late_tree": parity_metrics(
                full_compact_out, full_base_out
            ),
        },
    }
    ...
    del polar_base_out, polar_compact_out, full_base_out, full_compact_out
    torch.cuda.empty_cache()
    return result
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Compact late-stage Polar tree predication benchmark for the "
            "current early-radii full fused score path."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current compact-late fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current compact-late fast paths require D=128, M=128, num_levels=4.")

    print("========== Compact late-stage Polar tree predication benchmark ==========")
    ...
    print()
    print(
        "[Note] Baseline is the current FP16 L2 LUT + lane-nibble QJL + "
        "early-radii path. The candidate packs L3 outputs into lanes 0..15 "
        "and L4/radius outputs into lanes 0..7."
    )
    ...
    payload = {
        "benchmark": "polarquant_tree_late_compact_predication",
        "method": "early_radii_vs_compact_late_tree_lane_mapping",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] Compact late-stage Polar tree predication benchmark completed.")
