...
def benchmark_one_length(
    *,
    ...
    def full_l1_lane_nibble() -> torch.Tensor:
    ...
    def full_l2_lane_nibble() -> torch.Tensor:
    ...
    polar_l1_ms, polar_l1_scores = bench_cuda_ms(
        polar_l1,
        warmup=warmup,
        iters=iters,
    )
    polar_l2_ms, polar_l2_scores = bench_cuda_ms(
        polar_l2,
        warmup=warmup,
        iters=iters,
    )
    full_l1_ms, full_l1_scores = bench_cuda_ms(
        full_l1_lane_nibble,
        warmup=warmup,
        iters=iters,
    )
    full_l2_ms, full_l2_scores = bench_cuda_ms(
        full_l2_lane_nibble,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            "polar_only_l1_factor_lut_ms": float(polar_l1_ms),
            "polar_only_l2_combined_factor_lut_ms": float(polar_l2_ms),
            "full_lane_nibble_l1_factor_lut_ms": float(full_l1_ms),
            "full_lane_nibble_l2_combined_factor_lut_ms": float(full_l2_ms),
        },
        "speedup": {
            "polar_l2_over_l1": float(polar_l1_ms / polar_l2_ms),
            "full_l2_over_l1": float(full_l1_ms / full_l2_ms),
        },
        "parity": {
            "polar_l2_vs_l1": parity_metrics(
                polar_l2_scores,
                polar_l1_scores,
            ),
            "full_l2_vs_l1": parity_metrics(
                full_l2_scores,
                full_l1_scores,
            ),
        },
    ...
    del q_projected, l1_factor_lut, l2_factor_lut, lane_nibble_qjl_signs
    del polar_l1_scores, polar_l2_scores, full_l1_scores, full_l2_scores
    torch.cuda.empty_cache()
    return result
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "On-demand factor lookup reduction benchmark: "
            "L1 factor LUT vs L2 combined factor LUT, "
            "for Polar-only and full lane-nibble fused score."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current L2 combined LUT fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError("Current L2 combined LUT fast paths require D=128, M=128, num_levels=4.")

    print("========== L2 combined factor LUT lookup reduction benchmark ==========")
    ...
    print()
    print(
        "[Note] L2 combined LUT replaces "
        "2 x L1 factor lookup + L2 arithmetic combine "
        "with 1 x combined lookup indexed by (c1a,c1b,c2)."
    )
    ...
    payload = {
        "benchmark": "polarquant_l2_combined_factor_lut_lookup_reduction",
        "method": "l1_factor_lut_vs_l2_combined_factor_lut_lane_nibble",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] L2 combined factor LUT lookup reduction benchmark completed.")
