...
def main():
    ...
    scores_ref = turboquant_decode_score_transposed_cuda(
        q_rot=q_rot,
        q_sketch=q_sketch,
    ...
    scores_sharedq = turboquant_decode_score_transposed_sharedq_cuda(
        q_rot=q_rot,
        q_sketch=q_sketch,
        packed_mse_indices_t=packed_mse_t,
        mse_norms=mse_norms,
    ...
    diff = torch.abs(scores_ref - scores_sharedq)

    print("========== Shared-Q transposed TurboQuant score parity ==========")
    print(f"scores_ref.shape     = {tuple(scores_ref.shape)}")
    print(f"scores_sharedq.shape  = {tuple(scores_sharedq.shape)}")
    print(f"max_abs_diff          = {float(diff.max().item()):.6e}")
    print(f"mean_abs_diff         = {float(diff.mean().item()):.6e}")

    assert float(diff.max().item()) < 5e-5

    print("[PASS] Shared-Q transposed TurboQuant score parity passed.")
