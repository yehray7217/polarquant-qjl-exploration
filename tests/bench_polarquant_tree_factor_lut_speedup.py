...
from turboquant.turboquant_logits_tree_lut_cuda import (
    ...
    turboquant_polar_tree_l2_lut_fused_logits_cuda,
)
from turboquant.polar_tree_lut import (
    build_tree_l1_factor_lut,
    build_tree_l2_factor_lut,
)
...
def safe_ratio(a: float, b: float) -> float:
    return float("inf") if b == 0.0 else float(a / b)
...
def benchmark_one_length(
    *,
    ...
    # Query-side LUTs are built once and reused across all compressed keys
    # for this decode step.
    l1_factor_lut = build_tree_l1_factor_lut(
        q=q_fp32,
        centroids_l1=codebooks.centroids[0],
    )
    l2_factor_lut = build_tree_l2_factor_lut(
        q=q_fp32,
        centroids_l1=codebooks.centroids[0],
    ...
    def l1_lut_logits() -> torch.Tensor:
    ...
    def l2_lut_logits() -> torch.Tensor:
        return turboquant_polar_tree_l2_lut_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
    ...
    def build_l1_lut() -> torch.Tensor:
        return build_tree_l1_factor_lut(
            q=q_fp32,
            centroids_l1=codebooks.centroids[0],
        )

    def build_l2_lut() -> torch.Tensor:
        return build_tree_l2_factor_lut(
            q=q_fp32,
            centroids_l1=codebooks.centroids[0],
            centroids_l2=codebooks.centroids[1],
        )

    dense_fp16_ms, _ = bench_cuda_ms(
        dense_fp16_einsum,
        warmup=warmup,
        iters=iters,
    )
    dense_fp32_ms, dense_fp32_scores = bench_cuda_ms(
        dense_fp32_qkt,
        warmup=warmup,
        iters=iters,
    )
    ...
    l1_lut_score_ms, l1_scores = bench_cuda_ms(
        l1_lut_logits,
        warmup=warmup,
        iters=iters,
    )
    l2_lut_score_ms, l2_scores = bench_cuda_ms(
        l2_lut_logits,
        warmup=warmup,
        iters=iters,
    )
    l1_lut_build_ms, _ = bench_cuda_ms(
        build_l1_lut,
        warmup=warmup,
        iters=iters,
    )
    l2_lut_build_ms, _ = bench_cuda_ms(
        build_l2_lut,
        warmup=warmup,
        iters=iters,
    )

    l1_total_ms = l1_lut_build_ms + l1_lut_score_ms
    l2_total_ms = l2_lut_build_ms + l2_lut_score_ms
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "tree_l1_lut_score_ms": float(l1_lut_score_ms),
            "tree_l2_lut_score_ms": float(l2_lut_score_ms),
            "tree_l1_lut_build_ms": float(l1_lut_build_ms),
            "tree_l2_lut_build_ms": float(l2_lut_build_ms),
            "tree_l1_lut_build_plus_score_ms": float(l1_total_ms),
            "tree_l2_lut_build_plus_score_ms": float(l2_total_ms),
        },
        "speedup": {
            "tree_over_dense_fp32_qkt": float(dense_fp32_ms / tree_ms),
            "l1_lut_score_over_tree": float(tree_ms / l1_lut_score_ms),
            "l2_lut_score_over_tree": float(tree_ms / l2_lut_score_ms),
            "l1_lut_score_over_dense_fp32_qkt": float(dense_fp32_ms / l1_lut_score_ms),
            "l2_lut_score_over_dense_fp32_qkt": float(dense_fp32_ms / l2_lut_score_ms),
            "l1_lut_total_over_dense_fp32_qkt": float(dense_fp32_ms / l1_total_ms),
            "l2_lut_total_over_dense_fp32_qkt": float(dense_fp32_ms / l2_total_ms),
        },
        "quality_vs_dense_fp32_qkt": {
            "tree_fused_logits": score_error_metrics(
                scores_ref=dense_fp32_scores,
                scores_test=tree_scores,
            ),
            "tree_l1_lut_logits": score_error_metrics(
                scores_ref=dense_fp32_scores,
                scores_test=l1_scores,
            ),
            "tree_l2_lut_logits": score_error_metrics(
                scores_ref=dense_fp32_scores,
                scores_test=l2_scores,
            ),
        },
        "parity_vs_tree": {
            "l1_lut": parity_metrics(l1_scores, tree_scores),
            "l2_lut": parity_metrics(l2_scores, tree_scores),
        },
        "query_lut_bytes": {
            "l1_factor_lut_bytes": int(tensor_bytes(l1_factor_lut)),
            "l2_factor_lut_bytes": int(tensor_bytes(l2_factor_lut)),
        },
        "memory_bytes": {
            "dense_fp16_k_bytes": int(tensor_bytes(k_fp16)),
            "dense_fp32_k_bytes": int(tensor_bytes(k_fp32)),
        },
    }

    print(json.dumps(result, indent=2))

    del q_fp32, k_fp32, q_fp16, k_fp16
    del encoding, packed, packed_qjl_signs, qjl_norms, packed_meta, q_projected
    del l1_factor_lut, l2_factor_lut
    del dense_fp32_scores, tree_scores, l1_scores, l2_scores
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PolarQuant tree factor LUT benchmark: baseline tree vs L1 LUT vs L2 LUT."
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current tree LUT fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current tree LUT fast paths require D=128, M=128, num_levels=4."
        )

    print("========== PolarQuant tree factor LUT benchmark ==========")
    ...
    print()
    print(
        "[Note] Keeps 5.125-physical-bpc meta64 PolarQuant fixed. "
        "L1 and L2 query-side factor LUTs are built once per decode query; "
        "the JSON reports both LUT-score-only latency and LUT-build-plus-score latency."
    )
    ...
    payload = {
        "benchmark": "polarquant_tree_factor_lut_speedup",
        "method": "polarquant_meta64_m128_bits_4222_tree_factor_lut",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] PolarQuant tree factor LUT benchmark completed.")
