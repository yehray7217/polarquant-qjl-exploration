...
from turboquant.turboquant_meta_early_load_cuda import (
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda,
)
...
def benchmark_one_length(
    *,
    ...
    def candidate() -> torch.Tensor:
        return turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda(
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
            l2_factor_lut_fp16=l2_lut_fp16,
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "full_early_radii_early_qjl_norm_early_meta_ms": float(candidate_ms),
        },
        "speedup_vs_early_qjl_norm_baseline": {
            "full_early_meta_over_baseline": float(
                baseline_ms / candidate_ms
            ),
        },
        "parity_vs_early_qjl_norm_baseline": {
            "full_early_meta": parity_metrics(
                candidate_out,
                baseline_out,
            ),
        },
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Early packed-meta word load benchmark for the current "
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current early-meta fast path requires B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current early-meta fast path requires "
            "D=128, M=128, num_levels=4."
        )

    print("========== Early packed-meta word load benchmark ==========")
    ...
    print(
        ...
        "+ early-qjl-norm. Candidate hoists the packed-meta 32-bit word load "
        "before those scalar loads to increase load-use distance before "
        "shuffle/decode."
    )
    ...
    payload = {
        "benchmark": "polarquant_early_meta_word_load",
        "method": (
            "early_qjl_norm_baseline_vs_"
            "early_qjl_norm_plus_early_packed_meta_word_load"
        ),
        "device": str(device),
        "config": {
            "B": B,
    ...
    print("[PASS] Early packed-meta word load benchmark completed.")
