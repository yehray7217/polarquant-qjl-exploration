...
from turboquant.turboquant_meta_decode_shuffle_ablation_cuda import (
    turboquant_probe_no_l1_meta_decode_cuda,
    turboquant_probe_no_l2_meta_decode_cuda,
    turboquant_probe_no_l1_l2_meta_decode_cuda,
)
...
@torch.no_grad()
def output_difference(a: torch.Tensor, b: torch.Tensor) -> dict[str, float]:
...
def benchmark_one_length(
    *,
    ...
    def probe_no_l1() -> torch.Tensor:
        return turboquant_probe_no_l1_meta_decode_cuda(**common)

    def probe_no_l2() -> torch.Tensor:
        return turboquant_probe_no_l2_meta_decode_cuda(**common)

    def probe_no_l1_l2() -> torch.Tensor:
        return turboquant_probe_no_l1_l2_meta_decode_cuda(**common)
    ...
    no_l1_ms, no_l1_out = bench_cuda_ms(
        probe_no_l1,
        warmup=warmup,
        iters=iters,
    )
    no_l2_ms, no_l2_out = bench_cuda_ms(
        probe_no_l2,
        warmup=warmup,
        iters=iters,
    )
    no_l1_l2_ms, no_l1_l2_out = bench_cuda_ms(
        probe_no_l1_l2,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "probe_no_l1_meta_decode_ms": float(no_l1_ms),
            "probe_no_l2_meta_decode_ms": float(no_l2_ms),
            "probe_no_l1_l2_meta_decode_ms": float(no_l1_l2_ms),
        },
        "speedup_vs_baseline": {
            "no_l1_meta_decode_over_baseline": float(baseline_ms / no_l1_ms),
            "no_l2_meta_decode_over_baseline": float(baseline_ms / no_l2_ms),
            "no_l1_l2_meta_decode_over_baseline": float(baseline_ms / no_l1_l2_ms),
        },
        "difference_vs_baseline": {
            "probe_no_l1_meta_decode": output_difference(no_l1_out, baseline_out),
            "probe_no_l2_meta_decode": output_difference(no_l2_out, baseline_out),
            "probe_no_l1_l2_meta_decode": output_difference(no_l1_l2_out, baseline_out),
        },
        "probe_notes": {
            "probe_no_l1_meta_decode": "Output intentionally differs; use latency/NCU only.",
            "probe_no_l2_meta_decode": "Output intentionally differs; use latency/NCU only.",
            "probe_no_l1_l2_meta_decode": "Output intentionally differs; use latency/NCU only.",
        },
    }
    ...
    del baseline_out, no_l1_out, no_l2_out, no_l1_l2_out
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Packed-meta decode shuffle ablation benchmark for the current "
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current meta-decode probes require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current meta-decode probes require D=128, M=128, num_levels=4."
        )

    print("========== Packed-meta decode shuffle ablation benchmark ==========")
    ...
    print(
        ...
        "+ early-qjl-norm. Probes intentionally change outputs and are for "
        "latency/NCU attribution only: remove L1 decode, remove L2 decode, "
        "or remove both."
    )
    ...
    payload = {
        "benchmark": "polarquant_meta_decode_shuffle_ablation",
        "method": (
            "current_best_vs_no_l1_meta_decode_vs_no_l2_meta_decode_"
            "vs_no_l1_l2_meta_decode"
        ),
        "device": str(device),
        "config": {
            "B": B,
    ...
    print("[PASS] Packed-meta decode shuffle ablation benchmark completed.")
