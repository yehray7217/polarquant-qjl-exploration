...
from turboquant.turboquant_qjl_sign_layout_cuda import (
...
def benchmark_one_length(
    *,
    ...
    def qjl_only_meta64() -> torch.Tensor:
    ...
    def qjl_only_split_words() -> torch.Tensor:
        return turboquant_qjl_only_split_sign_words_cuda(
            q_projected=q_projected,
            packed_qjl_signs=packed_qjl_signs,
            qjl_norms=qjl_norms,
        )

    def qjl_only_lane_nibbles() -> torch.Tensor:
        return turboquant_qjl_only_lane_nibble_signs_cuda(
            q_projected=q_projected,
            lane_nibble_qjl_signs=lane_nibble_qjl_signs,
            qjl_norms=qjl_norms,
        )

    meta64_ms, meta64_scores = bench_cuda_ms(
        qjl_only_meta64,
        warmup=warmup,
        iters=iters,
    )
    split_words_ms, split_words_scores = bench_cuda_ms(
        qjl_only_split_words,
        warmup=warmup,
        iters=iters,
    )
    lane_nibbles_ms, lane_nibbles_scores = bench_cuda_ms(
        qjl_only_lane_nibbles,
        warmup=warmup,
        iters=iters,
    )
    ...
    result = {
        ...
        "timing_ms": {
            "qjl_only_meta64_ms": float(meta64_ms),
            "qjl_only_split_sign_words_ms": float(split_words_ms),
            "qjl_only_lane_nibble_signs_ms": float(lane_nibbles_ms),
        },
        "speedup": {
            "split_sign_words_over_meta64": float(meta64_ms / split_words_ms),
            "lane_nibble_signs_over_meta64": float(meta64_ms / lane_nibbles_ms),
            "lane_nibble_signs_over_split_sign_words": float(split_words_ms / lane_nibbles_ms),
        },
        "parity_vs_meta64_qjl_only": {
            "split_sign_words": parity_metrics(split_words_scores, meta64_scores),
            "lane_nibble_signs": parity_metrics(lane_nibbles_scores, meta64_scores),
        },
        "sign_storage_bytes": {
            "original_packed_qjl_signs_bytes": int(tensor_bytes(packed_qjl_signs)),
            "lane_nibble_qjl_signs_bytes": int(tensor_bytes(lane_nibble_qjl_signs)),
        },
    }
    ...
    del meta64_scores, split_words_scores, lane_nibbles_scores
    torch.cuda.empty_cache()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "QJL sign-bit lookup layout ablation: "
            "meta64 embedded signs vs split words vs lane-major nibbles."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current QJL sign-layout fast paths require B=1 and Q=1.")
    if D != 128 or M != 128 or num_levels != 4:
        raise ValueError(
            "Current QJL sign-layout fast paths require D=128, M=128, num_levels=4."
        )

    print("========== QJL sign-bit lookup layout ablation ==========")
    ...
    print()
    print(
        "[Note] All variants compute the same QJL residual contribution. "
        "Only the sign-bit physical layout / access pattern changes."
    )
    ...
    payload = {
        "benchmark": "qjl_sign_bit_lookup_layout_ablation",
        "method": "qjl_meta64_vs_split_words_vs_lane_nibbles",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] QJL sign-bit lookup layout ablation completed.")
