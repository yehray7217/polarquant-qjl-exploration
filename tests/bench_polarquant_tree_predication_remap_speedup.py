...
from turboquant.turboquant_logits_tree_predication_cuda import (
...
def benchmark_one_length(
    *,
    ...
    def polarquant_tree_remap_s4_logits() -> torch.Tensor:
        return turboquant_polar_tree_remap_s4_fused_logits_cuda(
            q=q_fp32,
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
    ...
    def polarquant_tree_remap_s34_logits() -> torch.Tensor:
        return turboquant_polar_tree_remap_s34_fused_logits_cuda(
            q=q_fp32,
            q_projected=q_projected,
            packed_meta=packed_meta,
            radii=encoding.polar.radii,
    ...
    dense_fp16_einsum_ms, _ = bench_cuda_ms(
    ...
    remap_s4_ms, remap_s4_scores = bench_cuda_ms(
        polarquant_tree_remap_s4_logits,
        warmup=warmup,
        iters=iters,
    )
    remap_s34_ms, remap_s34_scores = bench_cuda_ms(
        polarquant_tree_remap_s34_logits,
        warmup=warmup,
        iters=iters,
    )
    ...
    remap_s4_quality = score_error_metrics(
        scores_ref=dense_fp32_scores,
        scores_test=remap_s4_scores,
    )
    remap_s34_quality = score_error_metrics(
        scores_ref=dense_fp32_scores,
        scores_test=remap_s34_scores,
    )
    ...
    result = {
        ...
        "timing_ms": {
            ...
            "polarquant_tree_remap_s4_fused_logits_ms": float(remap_s4_ms),
            "polarquant_tree_remap_s34_fused_logits_ms": float(remap_s34_ms),
        },
        "speedup": {
            "tree_over_dense_fp32_qkt": float(dense_fp32_qkt_ms / tree_ms),
            "remap_s4_over_dense_fp32_qkt": float(dense_fp32_qkt_ms / remap_s4_ms),
            "remap_s34_over_dense_fp32_qkt": float(dense_fp32_qkt_ms / remap_s34_ms),
            "remap_s4_over_tree": float(tree_ms / remap_s4_ms),
            "remap_s34_over_tree": float(tree_ms / remap_s34_ms),
            "tree_over_coordinate": float(coordinate_ms / tree_ms),
            "remap_s4_over_coordinate": float(coordinate_ms / remap_s4_ms),
            "remap_s34_over_coordinate": float(coordinate_ms / remap_s34_ms),
        },
        "quality_vs_dense_fp32_qkt": {
            "coordinate_fused_logits": coordinate_quality,
            "tree_fused_logits": tree_quality,
            "tree_remap_s4_fused_logits": remap_s4_quality,
            "tree_remap_s34_fused_logits": remap_s34_quality,
        },
        "parity_vs_tree": {
            "remap_s4": parity_metrics(a=remap_s4_scores, b=tree_scores),
            "remap_s34": parity_metrics(a=remap_s34_scores, b=tree_scores),
        },
        "memory_bytes": {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "PolarQuant direct-tree predication-remapping benchmark: "
            "baseline tree vs remap_s4 vs remap_s34."
        )
    )
...
def main() -> None:
    ...
    if B != 1 or Q != 1:
        raise ValueError("Current tree remap CUDA fast path requires B=1 and Q=1.")
    if D != 128:
        raise ValueError(f"Current tree remap CUDA fast path requires D=128, got D={D}.")
    if M != 128:
        raise ValueError(f"Current tree remap CUDA fast path requires M=128, got M={M}.")
    if num_levels != 4:
        raise ValueError(f"Current tree remap CUDA fast path requires num_levels=4, got {num_levels}.")

    print("========== PolarQuant direct-tree predication-remap benchmark ==========")
    ...
    print()
    print(
        "[Note] Keeps the existing 5.125-physical-bpc PolarQuant meta64 format. "
        "Compares the validated direct-tree kernel against two predication-reduction "
        "experiments: remap_s4 and remap_s34."
    )
    ...
    payload = {
        "benchmark": "polarquant_tree_predication_remap_speedup",
        "method": "polarquant_meta64_m128_bits_4222_tree_predication_remap",
        "device": str(device),
        "config": {
            "B": B,
            "H": H,
    ...
    print("[PASS] PolarQuant direct-tree predication-remap benchmark completed.")
