...
__device__ __forceinline__ float l2_combined_s2(
    uint32_t meta_word_local,
    ...
    return l2_factor_lut[l2_base + combo];
}
...
__global__ void turboquant_polar_tree_l2_combined_lut_polar_only_decode_b1q1_warp8_meta64_kernel(
...
__global__ void turboquant_polar_tree_l2_combined_lut_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
...
void validate_meta(
    ...
    TORCH_CHECK(packed_meta.size(0) == 1, "L2 combined LUT kernels require B=1");
...
void validate_l2_lut(
    ...
    TORCH_CHECK(l2_factor_lut.size(0) == H, "l2_factor_lut H mismatch");
    TORCH_CHECK(l2_factor_lut.size(1) == L2_LUT_GROUPS, "l2_factor_lut group dim must be 32");
    TORCH_CHECK(l2_factor_lut.size(2) == L2_LUT_CODES, "l2_factor_lut code dim must be 1024");
}
...
void validate_grid(int64_t H, int64_t T) {
    ...
    TORCH_CHECK(groups <= 2147483647LL, "L2 combined LUT grid.x exceeds CUDA limit");
    TORCH_CHECK(H <= 65535, "L2 combined LUT grid.y exceeds CUDA limit");
}

} // namespace


torch::Tensor turboquant_polar_tree_l2_combined_lut_polar_only_cuda(
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut,
    torch::Tensor cos_l3,
    ...
    validate_l2_lut(l2_factor_lut, H);
    ...
    turboquant_polar_tree_l2_combined_lut_polar_only_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut,
    ...
    turboquant_polar_tree_l2_combined_lut_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_polar_tree_l2_combined_lut_polar_only_cuda,
        "Polar-only tree score with L2 combined factor LUT"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_lane_nibble_fused_logits_cuda,
        "Full fused logits with L2 combined factor LUT and lane-nibble QJL signs"
    );
}
