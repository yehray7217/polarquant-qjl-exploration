...
__global__ void turboquant_polar_tree_fused_logits_decode_b1q1_warp8_meta64_kernel(
    ...
    // The existing meta64 layout is unchanged. One key owns 16 uint32 words.
    ...
    // ---------------------------------------------------------------------
    // Polar Stage-1 direct tree dot product.
    //
    // D=128, L=4 => 8 independent 16-coordinate polar subtrees.
    // One warp has 32 lanes; each 16-coordinate subtree uses 4 lanes.
    //
    // local lane within subtree:
    //   lane4=0..3
    // block16:
    //   polar subtree index 0..7
    //
    // Recurrence:
    //   s1 = q_left * cos(theta1) + q_right * sin(theta1)
    //   s2 = s1_left * cos(theta2) + s1_right * sin(theta2)
    //   s3 = s2_left * cos(theta3) + s2_right * sin(theta3)
    //   s4 = s3_left * cos(theta4) + s3_right * sin(theta4)
    //   block contribution = radius * s4
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // QJL residual correction is intentionally kept identical in spirit to
    // the existing fused logits path, so the experiment isolates the new
    // Polar Stage-1 computation form.
    // ---------------------------------------------------------------------
...
torch::Tensor turboquant_polar_tree_fused_logits_cuda(
    torch::Tensor q,
    ...
    TORCH_CHECK(q.size(0) == 1, "tree fast path currently requires B=1");
    TORCH_CHECK(q.size(2) == 1, "tree fast path currently requires Q=1");
    ...
    TORCH_CHECK(token_groups <= 2147483647LL, "tree meta64 grid.x exceeds CUDA limit");
    TORCH_CHECK(H <= 65535, "tree meta64 grid.y exceeds CUDA limit");
    ...
    turboquant_polar_tree_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_polar_tree_fused_logits_cuda,
        "PolarQuant direct-tree fused logits CUDA fast path (meta64, M=128)"
    );
}
