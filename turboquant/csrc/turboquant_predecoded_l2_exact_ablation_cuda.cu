...
__device__ __forceinline__ float l2_combined_s2_fp16_predecoded_l2_exact(
    uint32_t meta_word_local,
    const uint8_t* __restrict__ predecoded_l2_codes_ptr,
    const half* __restrict__ l2_factor_lut_fp16,
    int h_idx,
    int block16,
    int lane4
) {
    // Exact-value L2 decode ablation:
    //   - c1a/c1b remain the original packed-meta L1 decode path
    //   - c2 is loaded from a predecoded uint8 tensor containing the exact
    //     original L2 code in {0,1,2,3}
    //
    // Therefore the L2 combined factor LUT index remains identical to baseline:
    //   combo = (c2 << 8) | (c1b << 4) | c1a
    //
    // Any latency difference isolates the cost of the original L2
    // packed-meta shuffle / bit-extract path, modulo the memory access cost of
    // the predecoded c2 tensor itself.
    const int l1_pair_base = block16 * 8 + lane4 * 2;
    const int l1_idx_a = l1_pair_base;
    const int l1_idx_b = l1_pair_base + 1;
    ...
    const uint8_t c2 = static_cast<uint8_t>(
        predecoded_l2_codes_ptr[l2_idx] & 0x03u
    );
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_predecoded_l2_exact_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const uint8_t* __restrict__ lane_nibble_qjl_signs,
    const half* __restrict__ qjl_norms,
    const uint8_t* __restrict__ predecoded_l2_codes,
    float* __restrict__ out,
    int H,
    int T
) {
    ...
    const uint8_t* lane_signs_ptr =
        lane_nibble_qjl_signs + key_linear * QJL_LANE_SIGN_BYTES;
    const uint8_t* predecoded_l2_ptr =
        predecoded_l2_codes + key_linear * 32;
    ...
    const float s2 = l2_combined_s2_fp16_predecoded_l2_exact(
        meta_word_local,
        predecoded_l2_ptr,
        l2_factor_lut_fp16,
        h_idx,
...
void validate_predecoded_l2_codes(
    torch::Tensor predecoded_l2_codes,
    int64_t H,
    int64_t T
) {
    TORCH_CHECK(predecoded_l2_codes.is_cuda(), "predecoded_l2_codes must be CUDA");
    TORCH_CHECK(predecoded_l2_codes.dtype() == torch::kUInt8, "predecoded_l2_codes must be uint8");
    TORCH_CHECK(predecoded_l2_codes.dim() == 4, "predecoded_l2_codes must be [B,H,T,32]");
    TORCH_CHECK(predecoded_l2_codes.size(0) == 1, "predecoded_l2_codes B mismatch");
    TORCH_CHECK(predecoded_l2_codes.size(1) == H, "predecoded_l2_codes H mismatch");
    TORCH_CHECK(predecoded_l2_codes.size(2) == T, "predecoded_l2_codes T mismatch");
    TORCH_CHECK(predecoded_l2_codes.size(3) == 32, "predecoded_l2_codes last dim must be 32");
}
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_predecoded_l2_exact_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    validate_predecoded_l2_codes(predecoded_l2_codes, H, T);
    validate_grid(H, T);
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_predecoded_l2_exact_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
        ...
        lane_nibble_qjl_signs.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(qjl_norms.contiguous().data_ptr<at::Half>()),
        predecoded_l2_codes.contiguous().data_ptr<uint8_t>(),
        out.data_ptr<float>(),
        static_cast<int>(H),
        static_cast<int>(T)
    );
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_predecoded_l2_exact_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_predecoded_l2_exact_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + early radii + early qjl norm + exact-value predecoded L2 codes"
    );
    m.def(
