...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    // The original fast path loaded packed_meta words for lane < 16 even
    // though Polar decode in this kernel only consumes words 0..11:
    //   L1: 0..7, L2: 8..9, L3: 10, L4: 11.
    // This candidate keeps the same load timing but trims unused words 12..15.
    uint32_t meta_word_local = 0;
    if (lane < 12) {
        meta_word_local = reinterpret_cast<const uint32_t*>(meta_ptr)[lane];
    }
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    // Segment 1: early L1/L2-only meta prefetch.
    // These are exactly the words consumed by l2_combined_s2_fp16():
    //   L1 angle bytes -> words 0..7
    //   L2 angle bytes -> words 8..9
    // This is the stage whose first __shfl_sync() was the largest remaining
    // source-level long-scoreboard consumer.
    uint32_t meta_word_l1l2_local = 0;
    if (lane < 10) {
        meta_word_l1l2_local =
            reinterpret_cast<const uint32_t*>(meta_ptr)[lane];
    }

    // Keep the already validated early qjl_norm optimization.
    float residual_norm_float = 0.0f;
    if (lane == 0) {
        residual_norm_float = __half2float(qjl_norms[key_linear]);
    }
    ...
    // Keep the already validated early radii optimization.
    float radius_float = 0.0f;
    if (lane4 == 0) {
        radius_float = __half2float(radii_ptr[block16]);
    }
    ...
    // Segment 2: late L3/L4-only meta load.
    // Only words 10 and 11 are required by finish_tree_polar_with_radius():
    //   L3 code word -> owner lane 10
    //   L4 code word -> owner lane 11
    uint32_t meta_word_l3l4_local = 0;
    if (lane >= 10 && lane < 12) {
        meta_word_l3l4_local =
            reinterpret_cast<const uint32_t*>(meta_ptr)[lane];
    }
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_trimmed_meta12_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + early radii + early qjl norm + trimmed packed-meta words 0..11"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_segmented_meta_l1l2_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + early radii + early qjl norm + segmented packed-meta decode"
    );
    m.def(
