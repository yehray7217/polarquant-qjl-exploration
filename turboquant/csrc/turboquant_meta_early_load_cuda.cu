...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    // Source-level NCU showed that the largest remaining long-scoreboard
    // consumer was the first warp shuffle that depends on meta_word_local.
    // The baseline loaded packed meta immediately before L2/POLAR decode.
    // Hoist the packed-meta 32-bit word load here, before the already validated
    // early qjl_norm and early radii loads, to increase meta load-use distance.
    uint32_t meta_word_local = 0;
    if (lane < 16) {
        meta_word_local = reinterpret_cast<const uint32_t*>(meta_ptr)[lane];
    }
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        ...
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + lane-nibble QJL with early radii, early qjl norm, and early packed-meta word load"
    );
    m.def(
