...
constexpr int QJL_M64_SIGN_BYTES = 8;
constexpr int QJL_M32_SIGN_BYTES = 4;

__device__ __forceinline__ float qjl_sum_compact_m64(
    const uint8_t* __restrict__ compact_qjl_signs_ptr,
    const float* __restrict__ sh_qproj,
    int lane
) {
    const uint8_t packed_lane4 = compact_qjl_signs_ptr[lane >> 2];
    const uint8_t lane_two_bits = static_cast<uint8_t>(
        (packed_lane4 >> ((lane & 3) * 2)) & 0x03u
    );
    ...
    for (int k = 0; k < 2; ++k) {
        const int sketch_idx = lane + 32 * k;
        const float qproj_val = sh_qproj[sketch_idx];
        const uint8_t sign_bit = static_cast<uint8_t>((lane_two_bits >> k) & 0x01u);
        qjl_acc += sign_bit ? qproj_val : -qproj_val;
    }
    return warp_reduce_sum(qjl_acc);
}

__device__ __forceinline__ float qjl_sum_compact_m32(
    const uint8_t* __restrict__ compact_qjl_signs_ptr,
    const float* __restrict__ sh_qproj,
    int lane
) {
    const uint8_t packed_lane8 = compact_qjl_signs_ptr[lane >> 3];
    const uint8_t sign_bit = static_cast<uint8_t>(
        (packed_lane8 >> (lane & 7)) & 0x01u
    );
    const float qproj_val = sh_qproj[lane];
    const float qjl_acc = sign_bit ? qproj_val : -qproj_val;
    return warp_reduce_sum(qjl_acc);
}

template<int M_DIM>
__device__ __forceinline__ float qjl_scale_for_m() {
    return QJL_CORRECTION_SCALE
        * sqrtf(3.14159265358979323846f / 2.0f)
        / sqrtf(static_cast<float>(M_DIM));
}

__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m64_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float* __restrict__ cos_l4,
    const float* __restrict__ sin_l4,
    const uint8_t* __restrict__ compact_qjl_signs,
    const half* __restrict__ qjl_norms,
    float* __restrict__ out,
    int H,
    int T
    ...
    __shared__ float sh_qproj[M64];
    const int64_t qproj_offset = static_cast<int64_t>(h_idx) * M64;
    if (tid < M64) {
        sh_qproj[tid] = q_projected[qproj_offset + tid];
    }
    __syncthreads();
    if (h_idx >= H || t_idx >= T) return;

    const int64_t key_linear = static_cast<int64_t>(h_idx) * T + t_idx;
    const uint8_t* meta_ptr = packed_meta + key_linear * META_BYTES;
    const half* radii_ptr = radii + key_linear * 8;
    const uint8_t* signs_ptr = compact_qjl_signs + key_linear * QJL_M64_SIGN_BYTES;

    float residual_norm_float = 0.0f;
    if (lane == 0) residual_norm_float = __half2float(qjl_norms[key_linear]);

    const int block16 = lane >> 2;
    const int lane4 = lane & 3;
    float radius_float = 0.0f;
    if (lane4 == 0) radius_float = __half2float(radii_ptr[block16]);

    uint32_t meta_word_local = 0;
    if (lane < 16) meta_word_local = reinterpret_cast<const uint32_t*>(meta_ptr)[lane];

    const float s2 = l2_combined_s2_fp16(meta_word_local, l2_factor_lut_fp16, h_idx, block16, lane4);
    const float polar_sum = finish_tree_polar_with_radius(
        s2, meta_word_local, radius_float, cos_l3, sin_l3, cos_l4, sin_l4, block16, lane4
    );
    const float qjl_sum = qjl_sum_compact_m64(signs_ptr, sh_qproj, lane);
    if (lane == 0) {
        out[static_cast<int64_t>(h_idx) * T + t_idx] =
            polar_sum + qjl_scale_for_m<M64>() * residual_norm_float * qjl_sum;
    }
}

__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m32_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    __shared__ float sh_qproj[M32];
    const int64_t qproj_offset = static_cast<int64_t>(h_idx) * M32;
    if (tid < M32) {
        sh_qproj[tid] = q_projected[qproj_offset + tid];
    }
    __syncthreads();
    ...
    const uint8_t* meta_ptr = packed_meta + key_linear * META_BYTES;
    const half* radii_ptr = radii + key_linear * 8;
    const uint8_t* signs_ptr = compact_qjl_signs + key_linear * QJL_M32_SIGN_BYTES;
    ...
    const float qjl_sum = qjl_sum_compact_m32(signs_ptr, sh_qproj, lane);
    if (lane == 0) {
        out[static_cast<int64_t>(h_idx) * T + t_idx] =
            polar_sum + qjl_scale_for_m<M32>() * residual_norm_float * qjl_sum;
    }
}

void validate_reduced_fused_extras(
    torch::Tensor q_projected,
    torch::Tensor compact_qjl_signs,
    torch::Tensor qjl_norms,
    int64_t H,
    int64_t T,
    int64_t M_dim,
    ...
    TORCH_CHECK(q_projected.is_cuda(), "q_projected must be CUDA");
    TORCH_CHECK(q_projected.dtype() == torch::kFloat32, "q_projected must be float32");
    TORCH_CHECK(q_projected.dim() == 4, "q_projected must be [B,H,Q,M]");
    TORCH_CHECK(q_projected.size(0) == 1, "q_projected B mismatch");
    TORCH_CHECK(q_projected.size(1) == H, "q_projected H mismatch");
    TORCH_CHECK(q_projected.size(2) == 1, "q_projected Q must be 1");
    TORCH_CHECK(q_projected.size(3) == M_dim, "q_projected M mismatch");

    TORCH_CHECK(compact_qjl_signs.is_cuda(), "compact_qjl_signs must be CUDA");
    TORCH_CHECK(compact_qjl_signs.dtype() == torch::kUInt8, "compact_qjl_signs must be uint8");
    TORCH_CHECK(compact_qjl_signs.dim() == 4, "compact_qjl_signs must be [B,H,T,bytes]");
    TORCH_CHECK(compact_qjl_signs.size(0) == 1, "compact_qjl_signs B mismatch");
    TORCH_CHECK(compact_qjl_signs.size(1) == H, "compact_qjl_signs H mismatch");
    TORCH_CHECK(compact_qjl_signs.size(2) == T, "compact_qjl_signs T mismatch");
    TORCH_CHECK(compact_qjl_signs.size(3) == sign_bytes, "compact_qjl_signs bytes mismatch");
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m64_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    validate_l2_lut_fp16(l2_factor_lut_fp16, H);
    validate_trig(cos_l3, sin_l3, cos_l4, sin_l4);
    validate_reduced_fused_extras(q_projected, compact_qjl_signs, qjl_norms, H, T, M64, QJL_M64_SIGN_BYTES);
    validate_grid(H, T);
    auto out = allocate_out(packed_meta, H, T);
    const dim3 block(THREADS);
    const dim3 grid(static_cast<unsigned int>((T + 7) / 8), static_cast<unsigned int>(H), 1);
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m64_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
        ...
        cos_l4.contiguous().data_ptr<float>(),
        sin_l4.contiguous().data_ptr<float>(),
        compact_qjl_signs.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(qjl_norms.contiguous().data_ptr<at::Half>()),
        out.data_ptr<float>(),
        static_cast<int>(H), static_cast<int>(T)
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    return out;
}

torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m32_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    validate_l2_lut_fp16(l2_factor_lut_fp16, H);
    validate_trig(cos_l3, sin_l3, cos_l4, sin_l4);
    validate_reduced_fused_extras(q_projected, compact_qjl_signs, qjl_norms, H, T, M32, QJL_M32_SIGN_BYTES);
    validate_grid(H, T);
    auto out = allocate_out(packed_meta, H, T);
    const dim3 block(THREADS);
    const dim3 grid(static_cast<unsigned int>((T + 7) / 8), static_cast<unsigned int>(H), 1);
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m32_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m64_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m64_fused_logits_cuda,
        "Full fused FP16 L2 LUT + early radii + early qjl norm + compact M=64 QJL"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m32_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_compact_m32_fused_logits_cuda,
        "Full fused FP16 L2 LUT + early radii + early qjl norm + compact M=32 QJL"
    );
    m.def(
