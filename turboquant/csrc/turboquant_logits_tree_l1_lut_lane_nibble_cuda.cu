...
__global__ void turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
...
void validate_inputs(
    ...
    TORCH_CHECK(q_projected.size(0) == 1, "lane-nibble fused path requires B=1");
    TORCH_CHECK(q_projected.size(2) == 1, "lane-nibble fused path requires Q=1");
    ...
    TORCH_CHECK(lane_nibble_qjl_signs.size(1) == q_projected.size(1), "lane_nibble_qjl_signs H mismatch");
    TORCH_CHECK(lane_nibble_qjl_signs.size(2) == packed_meta.size(2), "lane_nibble_qjl_signs T mismatch");
...
void validate_grid(int64_t H, int64_t T) {
    ...
    TORCH_CHECK(token_groups <= 2147483647LL, "lane-nibble fused grid.x exceeds CUDA limit");
    TORCH_CHECK(H <= 65535, "lane-nibble fused grid.y exceeds CUDA limit");
}

} // namespace


torch::Tensor turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l1_factor_lut,
    ...
    turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_polar_tree_l1_lut_lane_nibble_fused_logits_cuda,
        "PolarQuant tree L1-LUT fused logits with lane-nibble QJL signs"
    );
}
