...
__device__ __forceinline__ float l2_combined_s2_fp16_probe_no_l1_meta_decode(
    uint32_t meta_word_local,
    const half* __restrict__ l2_factor_lut_fp16,
    int h_idx,
    int block16,
    ...
    //
    // Removes the two L1 packed-meta shuffle/decode chains:
    //   c1a = load_l1_code_meta64(...)
    //   c1b = load_l1_code_meta64(...)
    //
    // Keeps the L2 packed-meta decode and the same L2 combined LUT access.
    const uint8_t c1a = 0;
    const uint8_t c1b = 0;
...
__device__ __forceinline__ float l2_combined_s2_fp16_probe_no_l2_meta_decode(
    uint32_t meta_word_local,
    const half* __restrict__ l2_factor_lut_fp16,
    int h_idx,
    int block16,
    ...
    // Latency/NCU probe only. Output intentionally differs.
    //
    // Keeps the two L1 packed-meta shuffle/decode chains, but removes:
    //   c2 = load_l2_code_meta64(...)
    const int l1_pair_base = block16 * 8 + lane4 * 2;
    const int l1_idx_a = l1_pair_base;
    const int l1_idx_b = l1_pair_base + 1;
...
__device__ __forceinline__ float l2_combined_s2_fp16_probe_no_l1_l2_meta_decode(
    const half* __restrict__ l2_factor_lut_fp16,
    int h_idx,
    int block16,
    int lane4
    ...
    // Latency/NCU probe only. Output intentionally differs.
    //
    // Removes all L1/L2 packed-meta shuffle/decode chains from the L2-combined
    // factor-LUT path while preserving the downstream tree/QJL structure.
    const uint8_t c1a = 0;
    const uint8_t c1b = 0;
    const uint8_t c2 = 0;
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float s2 = l2_combined_s2_fp16_probe_no_l1_meta_decode(
        meta_word_local,
        l2_factor_lut_fp16,
        h_idx,
        block16,
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l2_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float s2 = l2_combined_s2_fp16_probe_no_l2_meta_decode(
        meta_word_local,
        l2_factor_lut_fp16,
        h_idx,
        block16,
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_l2_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float s2 = l2_combined_s2_fp16_probe_no_l1_l2_meta_decode(
        l2_factor_lut_fp16,
        h_idx,
        block16,
        lane4
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_meta_decode_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l2_meta_decode_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l2_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_l2_meta_decode_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_l2_meta_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_meta_decode_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_meta_decode_lane_nibble_fused_logits_cuda,
        "Latency probe: remove L1 packed-meta shuffle/decode from the current best full fused path"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l2_meta_decode_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l2_meta_decode_lane_nibble_fused_logits_cuda,
        "Latency probe: remove L2 packed-meta shuffle/decode from the current best full fused path"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_l2_meta_decode_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_probe_no_l1_l2_meta_decode_lane_nibble_fused_logits_cuda,
        "Latency probe: remove both L1 and L2 packed-meta shuffle/decode from the current best full fused path"
    );
    m.def(
