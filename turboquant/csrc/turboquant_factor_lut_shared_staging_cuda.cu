...
constexpr int L1_LUT_FLOATS = L1_LUT_PAIRS * L1_LUT_CODES;  // 1024 floats = 4 KB
...
__device__ __forceinline__ void stage_l1_factor_lut_shared(
    float* __restrict__ sh_l1_lut,
    const float* __restrict__ l1_factor_lut,
    int h_idx,
    int tid
) {
    const int64_t head_base =
        static_cast<int64_t>(h_idx) * L1_LUT_FLOATS;

    #pragma unroll
    for (int i = tid; i < L1_LUT_FLOATS; i += THREADS) {
        sh_l1_lut[i] = l1_factor_lut[head_base + i];
    }
}
...
__device__ __forceinline__ float polar_tree_l1_lut_shared_sum(
    uint32_t meta_word_local,
    const half* __restrict__ radii_ptr,
    const float* __restrict__ sh_l1_lut,
    ...
    const float s1a = sh_l1_lut[l1_idx_a * L1_LUT_CODES + c1a];
    const float s1b = sh_l1_lut[l1_idx_b * L1_LUT_CODES + c1b];
...
__global__ void turboquant_polar_tree_l1_lut_shared_polar_only_decode_b1q1_warp8_meta64_kernel(
    ...
    __shared__ float sh_l1_lut[L1_LUT_FLOATS];

    stage_l1_factor_lut_shared(
        sh_l1_lut,
        l1_factor_lut,
        h_idx,
        tid
    ...
    const float polar_sum = polar_tree_l1_lut_shared_sum(
        meta_word_local,
        radii_ptr,
        sh_l1_lut,
        cos_l2,
...
__global__ void turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
...
void validate_meta(
    ...
    TORCH_CHECK(packed_meta.size(0) == 1, "shared-LUT kernels require B=1");
...
void validate_grid(int64_t H, int64_t T) {
    ...
    TORCH_CHECK(groups <= 2147483647LL, "shared-LUT grid.x exceeds CUDA limit");
    TORCH_CHECK(H <= 65535, "shared-LUT grid.y exceeds CUDA limit");
}

} // namespace


torch::Tensor turboquant_polar_tree_l1_lut_shared_polar_only_cuda(
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l1_factor_lut,
    torch::Tensor cos_l2,
    ...
    turboquant_polar_tree_l1_lut_shared_polar_only_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
torch::Tensor turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l1_factor_lut,
    ...
    turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_polar_tree_l1_lut_shared_polar_only_cuda,
        "Polar-only L1-LUT score with CTA shared-memory factor LUT staging"
    );
    m.def(
        "turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l1_lut_shared_lane_nibble_fused_logits_cuda,
        "Full lane-nibble fused logits with CTA shared-memory factor LUT staging"
    );
}
