...
__device__ __forceinline__ float compact_late_tree_polar_sum(
    float s2_distributed,
    uint32_t meta_word_local,
    float early_radius_compact,
    const float* __restrict__ cos_l3,
    const float* __restrict__ sin_l3,
    const float* __restrict__ cos_l4,
    const float* __restrict__ sin_l4,
    ...
    // All FULL_MASK warp collectives must be executed by all lanes.
    // The original compact-late experiment incorrectly called FULL_MASK
    // __shfl_sync() inside lane<16 / lane<8 branches, which is undefined
    // and can make the kernel hang.

    const int compact_l3_idx = lane & 15;
    const int compact_l3_block16 = compact_l3_idx >> 1;
    const int compact_l3_pair = compact_l3_idx & 1;
    const int original_left_lane =
        compact_l3_block16 * 4 + compact_l3_pair * 2;
    ...
    const float s2_right = __shfl_sync(
        FULL_MASK,
        s2_distributed,
        original_left_lane + 1
    );

    const uint8_t c3 = load_l3_code_meta64(
        meta_word_local,
        compact_l3_idx
    );

    const float s3_candidate =
        s2_left * cos_l3[c3]
        + s2_right * sin_l3[c3];

    const float s3_compact =
        (lane < 16) ? s3_candidate : 0.0f;

    const int compact_l4_idx = lane & 7;
    const int compact_left_l3_lane = compact_l4_idx * 2;

    const float s3_left = __shfl_sync(
        FULL_MASK,
        s3_compact,
        compact_left_l3_lane
    );
    const float s3_right = __shfl_sync(
        FULL_MASK,
        s3_compact,
        compact_left_l3_lane + 1
    );

    const uint8_t c4 = load_l4_code_meta64(
        meta_word_local,
        compact_l4_idx
    );

    const float s4_candidate =
        s3_left * cos_l4[c4]
        + s3_right * sin_l4[c4];

    const float polar_block_contrib =
        (lane < 8) ? (early_radius_compact * s4_candidate) : 0.0f;

    return warp_reduce_sum(polar_block_contrib);
}


__global__ void turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_decode_b1q1_warp8_meta64_kernel(
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    const float* __restrict__ cos_l3,
    ...
    float early_radius_compact = 0.0f;
    if (lane < 8) {
        early_radius_compact = __half2float(radii_ptr[lane]);
    }
    ...
    const float polar_sum = compact_late_tree_polar_sum(
        s2,
        meta_word_local,
        early_radius_compact,
        cos_l3,
        sin_l3,
        cos_l4,
        sin_l4,
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_cuda(
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    torch::Tensor cos_l3,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
        cos_l3.contiguous().data_ptr<float>(),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        ...
        &turboquant_polar_tree_l2_combined_lut_fp16_compact_late_polar_only_cuda,
        "Polar-only FP16 L2 LUT with compact late-stage Polar tree mapping"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_compact_late_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + lane-nibble QJL with compact late-stage Polar tree mapping"
    );
