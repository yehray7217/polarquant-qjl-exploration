...
template<int HOT_K>
__device__ __forceinline__ float l2_combined_s2_fp16_hot_combo_split(
    uint32_t meta_word_local,
    const half* __restrict__ l2_factor_lut_fp16,
    const int16_t* __restrict__ hot_combo_ids,
    const half* __restrict__ hot_combo_values,
    int h_idx,
    int block16,
    int lane4
) {
    ...
    const int64_t group_linear =
        static_cast<int64_t>(h_idx) * L2_LUT_GROUPS + l2_idx;
    const int64_t hot_base = group_linear * HOT_K;

    #pragma unroll
    for (int k = 0; k < HOT_K; ++k) {
        const int hot_combo = static_cast<int>(hot_combo_ids[hot_base + k]);
        if (combo == hot_combo) {
            return __half2float(hot_combo_values[hot_base + k]);
        }
    }

    const int64_t dense_idx = group_linear * L2_LUT_CODES + combo;
    return __half2float(l2_factor_lut_fp16[dense_idx]);
}
...
template<int HOT_K>
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_hot_combo_split_early_radii_early_qjl_norm_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float s2 = l2_combined_s2_fp16_hot_combo_split<HOT_K>(
        meta_word_local,
        l2_factor_lut_fp16,
        hot_combo_ids,
        hot_combo_values,
...
void validate_hot_combo_tables(
    torch::Tensor hot_combo_ids,
    torch::Tensor hot_combo_values,
    int64_t H,
    int64_t hot_k
) {
    TORCH_CHECK(hot_combo_ids.is_cuda(), "hot_combo_ids must be CUDA");
    TORCH_CHECK(hot_combo_values.is_cuda(), "hot_combo_values must be CUDA");
    TORCH_CHECK(hot_combo_ids.dtype() == torch::kInt16, "hot_combo_ids must be int16");
    TORCH_CHECK(hot_combo_values.dtype() == torch::kFloat16, "hot_combo_values must be float16");
    TORCH_CHECK(hot_combo_ids.dim() == 3, "hot_combo_ids must be [H,32,K]");
    TORCH_CHECK(hot_combo_values.dim() == 3, "hot_combo_values must be [H,32,K]");
    TORCH_CHECK(hot_combo_ids.size(0) == H, "hot_combo_ids H mismatch");
    TORCH_CHECK(hot_combo_values.size(0) == H, "hot_combo_values H mismatch");
    TORCH_CHECK(hot_combo_ids.size(1) == L2_LUT_GROUPS, "hot_combo_ids group dim must be 32");
    TORCH_CHECK(hot_combo_values.size(1) == L2_LUT_GROUPS, "hot_combo_values group dim must be 32");
    TORCH_CHECK(hot_combo_ids.size(2) == hot_k, "hot_combo_ids K mismatch");
    TORCH_CHECK(hot_combo_values.size(2) == hot_k, "hot_combo_values K mismatch");
}
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_hot_combo4_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    validate_radii(radii, H, T);
    validate_l2_lut_fp16(l2_factor_lut_fp16, H);
    validate_hot_combo_tables(hot_combo_ids, hot_combo_values, H, 4);
    validate_trig(cos_l3, sin_l3, cos_l4, sin_l4);
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_hot_combo_split_early_radii_early_qjl_norm_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<4><<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
        hot_combo_ids.contiguous().data_ptr<int16_t>(),
        reinterpret_cast<const half*>(hot_combo_values.contiguous().data_ptr<at::Half>()),
        cos_l3.contiguous().data_ptr<float>(),
        sin_l3.contiguous().data_ptr<float>(),
        cos_l4.contiguous().data_ptr<float>(),
        sin_l4.contiguous().data_ptr<float>(),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_hot_combo8_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    validate_radii(radii, H, T);
    validate_l2_lut_fp16(l2_factor_lut_fp16, H);
    validate_hot_combo_tables(hot_combo_ids, hot_combo_values, H, 8);
    validate_trig(cos_l3, sin_l3, cos_l4, sin_l4);
    validate_fused_extras(
        q_projected,
        lane_nibble_qjl_signs,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_hot_combo_split_early_radii_early_qjl_norm_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<8><<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_hot_combo4_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_hot_combo4_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + hot-combo K=4 split + early radii + early qjl norm"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_hot_combo8_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_hot_combo8_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + hot-combo K=8 split + early radii + early qjl norm"
    );
    m.def(
