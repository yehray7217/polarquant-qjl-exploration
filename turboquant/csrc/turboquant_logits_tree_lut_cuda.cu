...
__device__ __forceinline__ float qjl_sum_meta64(
    uint32_t meta_word_local,
...
__global__ void turboquant_polar_tree_l1_lut_fused_logits_decode_b1q1_warp8_meta64_kernel(
    ...
    const float qjl_sum = qjl_sum_meta64(
        meta_word_local,
        sh_qproj,
        lane
    );
...
__global__ void turboquant_polar_tree_l2_lut_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,

    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const float* __restrict__ l2_factor_lut,
    ...
    const float s2 = l2_factor_lut[l2_base + combo];
...
void validate_common(
    ...
    TORCH_CHECK(q_projected.size(0) == 1, "LUT tree path requires B=1");
    TORCH_CHECK(q_projected.size(2) == 1, "LUT tree path requires Q=1");
    ...
    TORCH_CHECK(packed_meta.size(0) == 1, "packed_meta B mismatch");
    TORCH_CHECK(packed_meta.size(1) == q_projected.size(1), "packed_meta H mismatch");
    ...
    TORCH_CHECK(radii.size(1) == q_projected.size(1), "radii H mismatch");
...
torch::Tensor allocate_out(
    ...
    return torch::empty(
        {1, q_projected.size(1), 1, packed_meta.size(2)},
        torch::TensorOptions()
...
void validate_grid(int64_t H, int64_t T) {
    ...
    TORCH_CHECK(token_groups <= 2147483647LL, "tree LUT grid.x exceeds CUDA limit");
    TORCH_CHECK(H <= 65535, "tree LUT grid.y exceeds CUDA limit");
}
...
torch::Tensor turboquant_polar_tree_l1_lut_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    ...
    validate_common(q_projected, packed_meta, radii, qjl_norms);
    ...
    TORCH_CHECK(l1_factor_lut.size(0) == q_projected.size(1), "l1_factor_lut H mismatch");
    TORCH_CHECK(l1_factor_lut.size(1) == L1_LUT_PAIRS, "l1_factor_lut pair dimension must be 64");
    TORCH_CHECK(l1_factor_lut.size(2) == L1_LUT_CODES, "l1_factor_lut code dimension must be 16");
    ...
    auto out = allocate_out(q_projected, packed_meta);
    ...
    turboquant_polar_tree_l1_lut_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
...
torch::Tensor turboquant_polar_tree_l2_lut_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut,
    torch::Tensor cos_l3,
    torch::Tensor sin_l3,
    torch::Tensor cos_l4,
    torch::Tensor sin_l4,
    ...
    TORCH_CHECK(l2_factor_lut.is_cuda(), "l2_factor_lut must be CUDA");
    TORCH_CHECK(l2_factor_lut.dtype() == torch::kFloat32, "l2_factor_lut must be float32");
    TORCH_CHECK(l2_factor_lut.dim() == 3, "l2_factor_lut must be [H,32,1024]");
    TORCH_CHECK(l2_factor_lut.size(0) == q_projected.size(1), "l2_factor_lut H mismatch");
    TORCH_CHECK(l2_factor_lut.size(1) == L2_LUT_GROUPS, "l2_factor_lut group dimension must be 32");
    TORCH_CHECK(l2_factor_lut.size(2) == L2_LUT_CODES, "l2_factor_lut code dimension must be 1024");
    ...
    turboquant_polar_tree_l2_lut_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        l2_factor_lut.contiguous().data_ptr<float>(),
        cos_l3.contiguous().data_ptr<float>(),
        sin_l3.contiguous().data_ptr<float>(),
        cos_l4.contiguous().data_ptr<float>(),
        sin_l4.contiguous().data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        &turboquant_polar_tree_l1_lut_fused_logits_cuda,
        "PolarQuant direct-tree fused logits with query-side L1 factor LUT"
    );
    m.def(
        "turboquant_polar_tree_l2_lut_fused_logits_cuda",
        &turboquant_polar_tree_l2_lut_fused_logits_cuda,
        "PolarQuant direct-tree fused logits with query-side L2 factor LUT"
    );
}
