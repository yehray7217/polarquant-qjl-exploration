...
__device__ __forceinline__ float qjl_signed_value_xor_signflip(
    float qproj_val,
    uint8_t sign_bit
) {
    // Baseline convention:
    //   sign_bit == 1 -> +qproj_val
    //   sign_bit == 0 -> -qproj_val
    //
    // Flip the IEEE-754 sign bit only when sign_bit == 0.
    // This preserves exact FP32 magnitude and should be numerically identical
    // to the baseline unary-negate path for finite values, including signed zero.
    const uint32_t flip_sign_mask =
        static_cast<uint32_t>((sign_bit ^ 0x01u) & 0x01u) << 31;
    return __uint_as_float(__float_as_uint(qproj_val) ^ flip_sign_mask);
}


__device__ __forceinline__ float qjl_sum_lane_nibble_shift_decode(
    const uint8_t* __restrict__ lane_nibble_signs_ptr,
    const float* __restrict__ sh_qproj,
    int lane
) {
    // Exact-parity nibble extraction rewrite.
    //
    // Baseline uses a low/high-nibble ternary:
    //   lane odd  -> high nibble
    //   lane even -> low nibble
    //
    // This variant uses a dynamic 0-or-4-bit shift instead, keeping the
    // downstream sign application identical to baseline. It isolates whether
    // the LDG.U8 -> nibble-select chain can compile into a shorter path.
    const uint8_t packed_nibble_pair = lane_nibble_signs_ptr[lane >> 1];
    const int nibble_shift = (lane & 1) << 2;
    const uint8_t lane_nibble =
        static_cast<uint8_t>((packed_nibble_pair >> nibble_shift) & 0x0Fu);
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        const uint8_t sign_bit =
            static_cast<uint8_t>((lane_nibble >> k) & 0x01u);
        qjl_acc += sign_bit ? qproj_val : -qproj_val;
    }
    return warp_reduce_sum(qjl_acc);
}
...
__device__ __forceinline__ float qjl_sum_lane_nibble_xor_signflip(
    const uint8_t* __restrict__ lane_nibble_signs_ptr,
    const float* __restrict__ sh_qproj,
    int lane
) {
    // Exact-parity sign-application rewrite.
    //
    // Keep the baseline low/high nibble selection, but replace:
    //   sign_bit ? qproj_val : -qproj_val
    // with a bitwise FP32 sign-bit flip. This isolates whether the arithmetic
    // sign-application dependent chain can be shortened.
    const uint8_t packed_nibble_pair = lane_nibble_signs_ptr[lane >> 1];
    const uint8_t lane_nibble =
        (lane & 1)
            ? static_cast<uint8_t>((packed_nibble_pair >> 4) & 0x0Fu)
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        const uint8_t sign_bit =
            static_cast<uint8_t>((lane_nibble >> k) & 0x01u);
        qjl_acc += qjl_signed_value_xor_signflip(qproj_val, sign_bit);
    }
    return warp_reduce_sum(qjl_acc);
}


__device__ __forceinline__ float qjl_sum_lane_nibble_shift_decode_xor_signflip(
    const uint8_t* __restrict__ lane_nibble_signs_ptr,
    const float* __restrict__ sh_qproj,
    int lane
) {
    // Combined exact-parity instruction-chain rewrite:
    //   1) dynamic shift nibble extraction
    //   2) bitwise FP32 sign-bit flip instead of ternary +/- selection
    //
    // Same storage, same lane-nibble layout, same mathematical result.
    const uint8_t packed_nibble_pair = lane_nibble_signs_ptr[lane >> 1];
    const int nibble_shift = (lane & 1) << 2;
    const uint8_t lane_nibble =
        static_cast<uint8_t>((packed_nibble_pair >> nibble_shift) & 0x0Fu);
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float qjl_sum = qjl_sum_lane_nibble_shift_decode(
        lane_signs_ptr,
        sh_qproj,
        lane
    );
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_xor_signflip_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float qjl_sum = qjl_sum_lane_nibble_xor_signflip(
        lane_signs_ptr,
        sh_qproj,
        lane
    );
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_xor_signflip_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    const float qjl_sum = qjl_sum_lane_nibble_shift_decode_xor_signflip(
        lane_signs_ptr,
        sh_qproj,
        lane
    );
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_xor_signflip_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_xor_signflip_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_xor_signflip_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_xor_signflip_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_lane_nibble_fused_logits_cuda,
        "Exact-parity full fused path with QJL sign nibble dynamic-shift decode"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_xor_signflip_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_xor_signflip_lane_nibble_fused_logits_cuda,
        "Exact-parity full fused path with QJL FP32 XOR sign-bit flip"
    );
    m.def(
        "turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_xor_signflip_lane_nibble_fused_logits_cuda",
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_shift_decode_xor_signflip_lane_nibble_fused_logits_cuda,
        "Exact-parity full fused path with QJL dynamic-shift nibble decode plus FP32 XOR sign-bit flip"
    );
    m.def(
