...
__device__ __forceinline__ float qjl_sum_lane_nibble_from_early_byte(
    uint8_t packed_nibble_pair_early,
    const float* __restrict__ sh_qproj,
    int lane
) {
    const uint8_t lane_nibble =
        (lane & 1)
            ? static_cast<uint8_t>((packed_nibble_pair_early >> 4) & 0x0Fu)
            : static_cast<uint8_t>(packed_nibble_pair_early & 0x0Fu);
...
__global__ void turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel(
    const float* __restrict__ q_projected,
    const uint8_t* __restrict__ packed_meta,
    const half* __restrict__ radii,
    const half* __restrict__ l2_factor_lut_fp16,
    ...
    // Source-level NCU showed that the lane-nibble sign-byte load and its
    // immediate sign decode remain a major long-scoreboard source.  Load the
    // byte here, before the Polar tree and QJL reduction, to increase load-use
    // distance without changing the physical layout or score math.
    const uint8_t packed_nibble_pair_early =
        lane_signs_ptr[lane >> 1];
    ...
    const float qjl_sum = qjl_sum_lane_nibble_from_early_byte(
        packed_nibble_pair_early,
        sh_qproj,
        lane
    );
...
torch::Tensor turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_cuda(
    torch::Tensor q_projected,
    torch::Tensor packed_meta,
    torch::Tensor radii,
    torch::Tensor l2_factor_lut_fp16,
    ...
    turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_decode_b1q1_warp8_meta64_kernel<<<grid, block>>>(
        q_projected.contiguous().data_ptr<float>(),
        packed_meta.contiguous().data_ptr<uint8_t>(),
        reinterpret_cast<const half*>(radii.contiguous().data_ptr<at::Half>()),
        reinterpret_cast<const half*>(l2_factor_lut_fp16.contiguous().data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        ...
        &turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_cuda,
        "Full fused FP16 L2 LUT + lane-nibble QJL with early radii, early qjl norm, and early QJL sign-byte load"
    );
    m.def(
