...
def _load_ext():
    ...
    src = root / "csrc" / "turboquant_meta_early_load_cuda.cu"

    _EXT = load(
        name="turboquant_meta_early_load_cuda_ext",
        sources=[str(src)],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
...
@torch.no_grad()
def turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda(
    *,
    ...
    """
    Current best full fused path plus early packed-meta word load:
      - L2 combined factor LUT, FP16 storage
      - lane-nibble QJL signs
      - early radii load
      - early qjl_norm load
      - packed meta 32-bit word load hoisted before those scalar loads
    """
    _validate_cuda(
    ...
    return ext.turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_meta_lane_nibble_fused_logits_cuda(
