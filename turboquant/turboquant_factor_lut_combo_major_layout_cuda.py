...
def _load_ext():
    ...
    src = root / "csrc" / "turboquant_factor_lut_combo_major_layout_cuda.cu"

    _EXT = load(
        name="turboquant_factor_lut_combo_major_layout_cuda_ext",
        sources=[str(src)],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
...
@torch.no_grad()
def turboquant_combo_major_lut_fused_logits_cuda(
    *,
    q_projected: torch.Tensor,
    packed_meta: torch.Tensor,
    radii: torch.Tensor,
    l2_factor_lut_combo_major_fp16: torch.Tensor,
    ...
    """
    Current best full fused score path with only the L2 factor LUT storage order
    changed from [H,32,1024] to [H,1024,32].
    """
    _validate_cuda(
        q_projected=q_projected,
        packed_meta=packed_meta,
        radii=radii,
        l2_factor_lut_combo_major_fp16=l2_factor_lut_combo_major_fp16,
    ...
    return ext.turboquant_polar_tree_l2_combined_lut_fp16_combo_major_layout_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda(
        q_projected.contiguous().to(torch.float32),
        packed_meta.contiguous(),
        radii.contiguous().to(torch.float16),
        l2_factor_lut_combo_major_fp16.contiguous().to(torch.float16),
        cos_l3,
        sin_l3,
        cos_l4,
        sin_l4,
