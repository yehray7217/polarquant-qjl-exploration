...
def _load_ext():
    ...
    src = root / "csrc" / "turboquant_factor_lut_hot_combo_split_cuda.cu"

    _EXT = load(
        name="turboquant_factor_lut_hot_combo_split_cuda_ext",
        sources=[str(src)],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
...
def _common_call(
    ...
    *,
    ...
    hot_combo_ids: torch.Tensor,
    hot_combo_values: torch.Tensor,
    ...
    _validate_cuda(
        ...
        hot_combo_ids=hot_combo_ids,
        hot_combo_values=hot_combo_values,
    ...
    return ext_fn(
        ...
        hot_combo_ids.contiguous().to(torch.int16),
        hot_combo_values.contiguous().to(torch.float16),
        cos_l3,
        sin_l3,
        cos_l4,
        sin_l4,
...
@torch.no_grad()
def turboquant_hot_combo4_fused_logits_cuda(**kwargs) -> torch.Tensor:
    ext = _load_ext()
    return _common_call(
        ext.turboquant_polar_tree_l2_combined_lut_fp16_hot_combo4_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda,
        **kwargs,
    )


@torch.no_grad()
def turboquant_hot_combo8_fused_logits_cuda(**kwargs) -> torch.Tensor:
    ext = _load_ext()
    return _common_call(
        ext.turboquant_polar_tree_l2_combined_lut_fp16_hot_combo8_early_radii_early_qjl_norm_lane_nibble_fused_logits_cuda,
        **kwargs,
    )
