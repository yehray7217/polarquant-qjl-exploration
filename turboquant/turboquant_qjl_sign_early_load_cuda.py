...
def _load_ext():
    ...
    src = root / "csrc" / "turboquant_qjl_sign_early_load_cuda.cu"

    _EXT = load(
        name="turboquant_qjl_sign_early_load_cuda_ext",
        sources=[str(src)],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
...
@torch.no_grad()
def turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_cuda(
    *,
    ...
    """
    Current best full fused path plus early QJL sign-byte load:
      - L2 combined factor LUT, FP16 storage
      - lane-nibble QJL signs
      - early radii load
      - early qjl_norm load
      - lane-nibble QJL sign byte loaded before Polar/QJL work
    """
    _validate_cuda(
    ...
    return ext.turboquant_polar_tree_l2_combined_lut_fp16_early_radii_early_qjl_norm_early_qjl_sign_lane_nibble_fused_logits_cuda(
